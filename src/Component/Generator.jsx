import React, {  useState } from 'react';
import '../App.css'
import 'bootstrap/dist/css/bootstrap.min.css';
import { useNavigate, useLocation } from 'react-router-dom';
import { Container } from 'react-bootstrap';
import { useClipboard} from 'use-clipboard-copy';
import fire from '../fire';
import {FacebookButton} from 'react-social';

export const Generator=() => {

 const navigate=useNavigate();
 const location=useLocation();
 const url=new URLSearchParams(location.search).get('url');
 const clipboard=useClipboard();
 const [copied, setCopied]=useState(false);

 const copyLink=()=> {
     clipboard.copy(url);
     setCopied(true);
 };
 const handleLogout=() => {
    navigate('/');
    fire.auth().signOut();
  }
const download =()=> {
    console.log(url);
    const link=document.createElement('a')
  link.href=url
    link.setAttribute('download', 'image.jpg')
    document.body.appendChild(link)
    link.click()
}
  
  return (
   <>
    <div className='Header'>
      Meme Generator    '
      <svg xmlns="http://www.w3.org/2000/svg" width="30" height="30" fill="currentColor" class="bi bi-emoji-laughing" viewBox="0 0 16 16">
  <path d="M8 15A7 7 0 1 1 8 1a7 7 0 0 1 0 14zm0 1A8 8 0 1 0 8 0a8 8 0 0 0 0 16z"/>
  <path d="M12.331 9.5a1 1 0 0 1 0 1A4.998 4.998 0 0 1 8 13a4.998 4.998 0 0 1-4.33-2.5A1 1 0 0 1 4.535 9h6.93a1 1 0 0 1 .866.5zM7 6.5c0 .828-.448 0-1 0s-1 .828-1 0S5.448 5 6 5s1 .672 1 1.5zm4 0c0 .828-.448 0-1 0s-1 .828-1 0S9.448 5 10 5s1 .672 1 1.5z"/>
</svg>' </div>
<div>
    {url && <img alt='meme' className='Img' src={url}/> }
</div>
<div>
<Container className= 'pt-4 d-flex align-items-center justify-content-center'>
      <button  className='button' onClick={()=>navigate('/home')}>Back Home</button> <div className='padding'> </div>
      <button className='button2 'onClick={copyLink}>
      {
          copied ? 'Already Copied' : 'Copy link'
      }
      </button>
    
      </Container>
      <div>
      <Container className= 'pt-4 d-flex align-items-center justify-content-center'>

                <FacebookButton className='button4' url={url} appId={357508752571986}>
          {
              "Share With Facebook"
          }
      </FacebookButton>
      <div className='padding2'> </div>
      <button className='button4 'onClick={download}> Save/Download</button>
      </Container>
      </div>
      <div>
      <Container className= 'pt-4 d-flex align-items-center justify-content-center'>
   <button className='button3 'onClick={handleLogout}> Logout</button>
</Container>
      </div>
      <br></br>
      <div className='Header'>
        Thank You For Visiting   '
        <svg xmlns="http://www.w3.org/2000/svg" width="30" height="30" fill="currentColor" class="bi bi-emoji-wink" viewBox="0 0 16 16">
  <path d="M8 15A7 7 0 1 1 8 1a7 7 0 0 1 0 14zm0 1A8 8 0 1 0 8 0a8 8 0 0 0 0 16z"/>
  <path d="M4.285 9.567a.5.5 0 0 1 .683.183A3.498 3.498 0 0 0 8 11.5a3.498 3.498 0 0 0 3.032-1.75.5.5 0 1 1 .866.5A4.498 4.498 0 0 1 8 12.5a4.498 4.498 0 0 1-3.898-2.25.5.5 0 0 1 .183-.683zM7 6.5C7 7.328 6.552 8 6 8s-1-.672-1-1.5S5.448 5 6 5s1 .672 1 1.5zm1.757-.437a.5.5 0 0 1 .68.194.934.934 0 0 0 .813.493c.339 0 .645-.19.813-.493a.5.5 0 1 1 .874.486A1.934 1.934 0 0 1 10.25 7.75c-.73 0-1.356-.412-1.687-1.007a.5.5 0 0 1 .194-.68z"/>
</svg>'
     </div>
</div>
   </>
  );
}